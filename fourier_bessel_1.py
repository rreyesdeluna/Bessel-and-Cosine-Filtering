...
def limited_loop(i, imax):
    while i > imax: i = i - imax
    return i
...
# fig = px.line(x = t, y = st, width = 700, height = 600)
...
# fft_chart.singlesignal_FFT(t_vec = t, y_vec = st, x_axis = [0, 1], save_fig = 'fourier_1')
...
scale0 = np.array([x * 1. for x in range(1, fb_zeros + 1)])
...
# fig.write_image("bessel_functions.png")
# fig.show()
...
# fig = go.Figure()
# for n in range(fb_zeros):
#     fig.add_trace(go.Scatter(x = z, y = ps[n,:], line = dict(color=colors[n]), name = 's' + str(n + 1)))
...
# fig.write_image("biorthogonal_baseb.png")
#fig.show()

# fft_chart.multisignal_FFT(t_vec = z, y_vec = ps.T, x_axis = [0, 3], save_fig = 'fourier_2b')
...
# for i in range(fb_zeros):
#     fig.append_trace(go.Scatter(x = tres, y = ires[i , :], line = dict(color=colors[i]), name = 's' + str(i + 1), legendgroup = '1'), row=1, col=1)
    
# for i in range(modes):
#     fig.append_trace(go.Scatter(x = tres, y = iires[i , :], line = dict(color=colors[iindx[i]]), name = 's' + str(iindx[i] + 1), legendgroup = '2'), row=2, col=1)

# # for i in range(modes):
# #     fig.append_trace(go.Scatter(x = tres, y = iires[i , :], line = dict(color=colors[i]), name = 's' + str(iindx[i] + 1), legendgroup = '2'), row=2, col=1)
...
# fig.update_traces(line = dict(width = 7))
# fig.write_image("monocomponents_b.png")
# fig.show()
...
for i in range(modes):
    ...
    print('freq: ' + str(freq_av), ' / damp: ' + str(damp_av))
    if i == 2: sys.exit()
...
# y_vec[: , 1] = jres
y_vec[: , 1] = mres
# fft_chart.dossignal_FFT(t_vec = tres, y_vec = y_vec, h_vec = ['Signal', 'Estimation'], x_axis = [0, 1], save_fig = 'fourier_3b')
